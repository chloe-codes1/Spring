...
public class UpdateServiceImpl implements UpdateService{
	...
	@Override
	public int updateEmp(int empNo, String eName, String job, int mgr) {
		...
		try {
			...
			// SQLException이 발생할 수 있으므로, Exception으로 한번에 catch 하지 말고 세분화하기
			System.err.println("UpdateServiceImpl - updateEmp() => " + e.getMessage());
		} catch (Exception e) {
			System.out.println("UpdateServiceImpl - updateEmp() => " + e.getMessage());
		}
	...
	//바로 위의 method를 overloading => parameter가 다름
	@Override
	public int updateEmp(Emp emp) {

		return updateEmp(emp.getEmpNo(), emp.geteName(), emp.getJob(), emp.getMgr());
	}
